...
from aindy_sdk.watcher.session_tracker import SessionEvent
...
class SignalEmitter:
    ...
    def _send_with_retry(self, batch: List[SessionEvent]) -> None:
        ...
        body = json.dumps({"signals": [asdict(e) for e in batch]}).encode()
        headers = {
        ...
        for attempt in range(1, _MAX_RETRIES + 1):
            try:
                ...
                with urlopen(req, timeout=_HTTP_TIMEOUT) as resp:
                    status = resp.status
                if status < 400:
                    logger.debug("Emitted %d signals (status=%d)", len(batch), status)
                    return
                logger.warning(
                    "Watcher signal POST failed (attempt=%d status=%d)",
                    attempt,
                    status,
                )
                ...
                logger.warning(
                    "Watcher signal POST unexpected error (attempt=%d): %s", attempt, exc
                )
